...
interface CalorieModeSheetProps {
  open: boolean;
  onClose: () => void;
  mode: 'net' | 'goal' | 'adjusted';
  onPick: (mode: 'net' | 'goal' | 'adjusted') => void;
}

export function CalorieModeSheet({ open, onClose, mode, onPick }: CalorieModeSheetProps) {
  const { t } = useTranslation();
  const options: Array<{ id: 'adjusted' | 'goal' | 'net'; title: string; desc: string }> = [
    {
      id: 'adjusted',
      title: t('profile.mode_adjusted', 'Weekly adjusted'),
    ...
    {
      id: 'goal',
      title: t('profile.mode_goal', 'Fixed daily goal'),
      desc: t('profile.mode_goal_desc', 'The same target every day, based on your goal.'),
    },
    {
      id: 'net',
      title: t('profile.mode_net', 'Net balance'),
