...
export default function OnboardingPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!form.autoCalculateBMR && !parseFloat(form.bmrKcal)) {
      setError("BMR is required when auto-calculate is off.");
      return;
    }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <p className="mt-1 text-sm text-gray-500">
        We need a few details before you can start tracking.
      </p>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input
              ...
              disabled={form.autoCalculateBMR}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:bg-gray-100 disabled:text-gray-400"
            />
          </div>
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              type="checkbox"
              checked={form.autoCalculateBMR}
              onChange={(e) => set("autoCalculateBMR", e.target.checked)}
          ...
          <div>
            ...
            <input
              ...
              disabled={form.autoCalculateBodyFat}
              className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:bg-gray-100 disabled:text-gray-400"
            />
          </div>
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              type="checkbox"
              checked={form.autoCalculateBodyFat}
              onChange={(e) => set("autoCalculateBodyFat", e.target.checked)}
              className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                disabled={form.autoCalculateProteinGoal}
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:bg-gray-100 disabled:text-gray-400"
              />
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              type="checkbox"
              checked={form.autoCalculateProteinGoal}
              onChange={(e) => set("autoCalculateProteinGoal", e.target.checked)}
              className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
            />
